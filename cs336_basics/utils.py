...
class SimpleMapReduce:
    ...
    def shuffle(self, mapped_data: List[Tuple[Any, Any]]) -> Dict[Any, List[Any]]:
        """Shuffle阶段: 按键分组"""
    ...
    def reduce(self, reducer: Callable, shuffled_data: Dict[Any, List[Any]]) -> Dict[Any, Any]:
        """Reduce阶段: 对每个键的值列表应用reducer函数"""
        with multiprocessing.Pool(processes=self.num_workers) as pool:
            items = list(shuffled_data.items())
            results = pool.starmap(reducer, items)
        
        return dict(results)
    
    def execute(self, data: List[Any], mapper: Callable, reducer: Callable) -> Dict[Any, Any]:
        """执行完整的MapReduce流程"""
        # Map阶段
        # print("执行Map阶段...")
